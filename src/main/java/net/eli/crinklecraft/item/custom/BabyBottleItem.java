...
/**
 * Baby bottle that can hold milk (clears effects when drunk) or up to 3 potions (all applied in one drink).
 */
public class BabyBottleItem extends Item {

    public static final int USE_DURATION_TICKS = 32;
    /** Immediate pee added on drink (0-100 gauge). */
    public static final float PEE_ON_DRINK = 8f;
    /** Multiplier applied to pee fill rate after drink. */
    public static final float PEE_BOOST_MULTIPLIER = 2.0f;
    /** Duration (ticks) of the pee fill boost after drink. */
    public static final int PEE_BOOST_DURATION_TICKS = 20 * 60; // 60s
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        // Try to fill from other hand first (so adding 2nd/3rd potion doesn't start drinking)
        ...
        if (contents.canAddPotion(ModDataComponentTypes.BottleContents.MAX_POTIONS) && other.has(DataComponents.POTION_CONTENTS)) {
            ...
            if (!fromPotion.isEmpty()) {
                ...
                stack.set(ModDataComponentTypes.BOTTLE_CONTENTS.get(),
                        new ModDataComponentTypes.BottleContents(false, newEffects, contents.potionCount + 1));
                if (!player.getAbilities().instabuild)
                    other.shrink(1);
                return InteractionResultHolder.success(stack);
            }
        ...
        // Only then: if bottle has contents, drink
        if (!contents.isEmpty()) {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag flag) {
        ...
        if (contents != null && !contents.isEmpty()) {
            if (contents.hasMilk) {
                ...
                tooltipComponents.add(Component.translatable("item.crinklecraft.baby_bottle.tooltip.potions", contents.potionCount, ModDataComponentTypes.BottleContents.MAX_POTIONS));
            }
        }
    }
}
